...
function checkAuth($obj){
  ...
  if($method !== 'POST'){
    ...
    if($obj['username']=="gjepc@tritiya" && $obj['password']=="gjepc@iijs2023"){
      return true;
    }else{
      return false;
    }
 ...
 // $event = trim($obj['event']);
  $datetime = date("Y-m-d H:i:s");
	...
	//	$sql = "SELECT * FROM gjepclivedatabase.globalExhibition where 1 AND pan_no='$pan_no' AND (participant_Type='VIS' || participant_Type='IGJME')";
		$sql = "SELECT * FROM gjepclivedatabase.globalExhibition where 1 AND pan_no='$pan_no' AND participant_Type='VIS'";
        ...
        /*  if($face_status =="Y")
          ...
          $isVerified = $getGlobal[0]->isVerified;
            ...
            $data = array("otp"=>$otp,"modified_date"=>$datetime,"otp_source"=>"app");
            ...
            if($result){            
              ...
              if($isSent){
                ...
                "Result"=>array("mobile_no"=>$mobile_no,"otp"=>$otp,"participant_Type"=>"VIS","company_name"=>$company_name,"visitor_name"=>$visitor_name,"designation"=>$designation),
           ...
           if($getOtpDetails !="NA"){            
            ...
            $result=$this->Mdl_badge->update("globalExhibition",array("pan_no"=>$pan_no,"id"=>$id,),$data);
              $uniqueId = trim($getOtpDetails[0]->uniqueIdentifier);
                ...
                $link  = "https://registration.gjepc.org/visitor-badge.php?action=generateBadge&source=app&uniqueIdentifier=".$uniqueId;
                $response=array(
        ...
        */
        ...
        if($emailExist ===TRUE){
         ...
         /*  $status =  $getGlobal[0]->status;
          if($status  =="Y"){ */
           $isVerified = $getGlobal[0]->isVerified;
		   $registration_id = $getGlobal[0]->registration_id;
           $visitor_id = $getGlobal[0]->visitor_id;
          // if($isVerified =="1"){
          ...
          $designation = $getGlobal[0]->designation;
            ...
            if($result){
               ...
               $message ='
                <table width="100%" bgcolor="#fbfbfb" style="margin:2% auto; border:2px solid #ddd; font-family:Arial, sans-serif; color:#333333; font-size:13px; border-radius:10px;">
                    ...
                    <td style="padding:30px;">  
                      ...
                      <tr>
                        ...
                        <td align="right"><img id="ri" src="https://gjepc.org/iijs-tritiya/assets/images/iijs_logo.png"></td>
                      </tr>
                      <tr>
                      ...
                      <p>Warm Regards,</p></br>
                      <p>IIJS TRITIYA 2023 SHOW Web Team.</p>
                      </td>
                      </tr>
                      <tr>
                ...
                $subject = "IIJS TRITIYA 2023 - Email ID Verification"; 
			    $cc = "";
			    $emailSend = array($email,"naheed@gjepcindia.com");
                ...
                $response=array(
                //  "verified"=>"N",
                  "Result"=>array("email"=>$email,"participant_Type"=>"INTL","company_name"=>$company_name,"visitor_name"=>$visitor_name,"designation"=>$designation,"otp"=>$otp,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id),
        ...
        /*} else {
			...
			//	"verified"=>"N",
				"Result"=>"",
				"Message"=>"You are not registered for IIJS TRITIYA 2023",
				"status"=>"false"
				);  
        }
  }else{
    ...
    if($mobile !=""){
        ...
        */
        ...
        if($getGlobal !=="NA"){
          ...
          //   ); 
          // }else{
            $message = "One Time Password for Exhibitor Verification is ".$otp." , Regards GJEPC";
            $data = array("otp"=>$otp,"modified_date"=>$datetime,"otp_source"=>"app");
            ...
            if($result){
              ...
              if($isSent){
                $response=array(
                  "Result"=>array("mobile_no"=>$mobile_no,"otp"=>$otp,"participant_Type"=>"Exhibitor","company_name"=>$company_name,"visitor_name"=>$visitor_name,"designation"=>$designation,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id),
                  "Message"=>"OTP has been Sent to Registered  mobile number",
                  "status"=>"true"
                   );
              }else{
          ...
          //}
            ...
            "verified"=>"N",
            "Result"=>"",
             "Message"=>"You are not registered for the show",
            "status"=>"false"
          );  
        }
    } else {
...
//  print_r($obj);exit;
  ...
  $datetime = date("Y-m-d H:i:s");
	...
	if($mobile !="" && $otp_number !=""){
        ...
        */
        $sql = "SELECT * FROM globalExhibition WHERE mobile = '$mobile' AND participant_Type ='EXH'";
        $mobileExist = $this->Mdl_badge->isExistCustom($sql);
        if($mobileExist ===TRUE){
           
           $getOtpDetails =  $this->Mdl_badge->customQuery("SELECT * FROM globalExhibition WHERE mobile ='$mobile' AND otp='$otp_number' AND participant_Type='EXH'");  
...
function send_sms_yellow($sendMobile,$otp)
{
	if(strlen($sendMobile)==10){  $mobile = '91'.$sendMobile; } else {  $mobile = $sendMobile;  }
	$otp = trim($otp);
...
function emailContent($to,$visName,$message){
 ...
 function generate_qr($uniqueId)
	...
	/* Visitor Login API for IIJS APP */
	/********************************************************** DOMESTIC Visitor PAN VEFIRICATION *******************************/
function verifyVisitor()
{
  ...
  $datetime = date("Y-m-d H:i:s");
    ...
    if($pan_no !=""){
        ...
        ** CHECK PAN NUMBER IS ALREADY EXIST IN TABLE OR NOT
        */
		$sql =  "SELECT * FROM visitor_directory WHERE pan_no='$pan_no' order by visitor_approval limit 1";
        $panExist = $this->Mdl_badge->isExistCustom($sql);
        if($panExist ===TRUE){
          $digits = 4;  
          $otp = rand(pow(10, $digits-1), pow(10, $digits)-1);
		
		$row = $this->Mdl_badge->customQuery($sql);
		
		$registration_id = $row[0]->registration_id;
		$getData = $this->Mdl_badge->retrieve("registration_master",array("id"=>$registration_id));
		$company_name = strtoupper(str_replace('&amp;', '&', $getData[0]->company_name));
		$mobile = $row[0]->mobile;
		$email_new = $row[0]->email;
		
		$designation_id = $row[0]->designation;
		$getDesgn = $this->Mdl_badge->retrieve("visitor_designation_master",array("id"=>$designation_id));
		$designation = $getDesgn[0]->type_of_designation;
		$name = $row[0]->name;
		$lname = $row[0]->lname;
		$visitor_name = $name.' '.$lname;
		$photo = $row[0]->photo;
		$visitor_id = $row[0]->visitor_id;
		$visitor_approval = $row[0]->visitor_approval;
		$photo_path = $row[0]->photo;
		if($visitor_approval =="Y")
		{
          $isSecondary = $row[0]->isSecondary;
          if($isSecondary =="Y"){
            $mobile_no = $row[0]->secondary_mobile;
          } else {
            $mobile_no = $row[0]->mobile;
          }

            $message = "One Time Password for Visitor Verification is ".$otp." , Regards GJEPC";          
            $data = array("otp"=>$otp,"verified"=>'0',"modifiedDate"=>$datetime);
            $insertData = array(
				"mobile_no"=>$mobile_no,
			...
			$sql = "SELECT * FROM visitor_mobileotpverification WHERE 1 AND mobile_no='$mobile_no' AND visitor_id='$visitor_id' AND registration_id = '$registration_id'";
			$mobileExist = $this->Mdl_badge->isExistCustom($sql);
			if($mobileExist ===TRUE){
			  $result = $this->Mdl_badge->update("visitor_mobileotpverification",array("mobile_no"=>$mobile_no,"visitor_id"=>$visitor_id,"registration_id"=>$registration_id),$data);
			} else {
			  $result = $this->Mdl_badge->insert("visitor_mobileotpverification",$insertData);
			//echo $this->db->last_query();  exit;
			}
			
		//	echo $this->db->last_query();  exit;
            if($result){            
            //$isSent = $this->send_sms_yellow($mobile_no,$otp);
			  $isSent = $this->send_otp($message,$mobile_no);
              if($isSent){
				$photo="https://registration.gjepc.org/images/employee_directory/".$registration_id."/photo/".$photo_path;
                $response=array(               
                "Result"=>array("participant_Type"=>"VIS","company_name"=>$company_name,"visitor_name"=>$visitor_name,"designation"=>$designation,"photo"=>$photo,"mobile_no"=>$mobile_no,"otp"=>$otp,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id),
                "Message"=>"OTP has been Sent to Registered mobile number",
                "status"=>"true"
                );
              }else{
              ...
              $response=array(               
                "Result"=>array(),
                "Message"=>"Error : PAN No may be Disapproved,Pending",
                "status"=>"false"
              );  
        } 
        } else {
...
function visitorverifyOtp(){
  $json = file_get_contents('php://input');
  $obj = json_decode($json,True);
  
  $registration_id = trim($obj['registration_id']);
  $visitor_id = trim($obj['visitor_id']);
  $otp = trim($obj['otp']);
  $mobile_no = trim($obj['mobile_no']);
  ...
  $datetime = date("Y-m-d H:i:s");
    ...
    if($mobile_no !="" && $otp !="")
    {
        ...
        ** CHECK MOBILE NUMBER IS ALREADY EXIST IN TABLE OR NOT
        */
		$sql = "SELECT * FROM visitor_mobileotpverification WHERE otp='$otp' AND mobile_no='$mobile_no' AND visitor_id='$visitor_id' AND registration_id = '$registration_id'";
        $panExist = $this->Mdl_badge->isExistCustom($sql);
        if($panExist ===TRUE){
		$data = array("registration_id"=>$registration_id,"visitor_id"=>$visitor_id);
           $getOtpDetails = $this->Mdl_badge->retrieve("visitor_mobileotpverification",array("mobile_no"=>$mobile_no,"otp"=>$otp,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id));    
		//	echo $this->db->last_query();  exit;		   
           if($getOtpDetails !="NA"){            
            $db_otp = trim($getOtpDetails[0]->otp);
            if($db_otp == $otp){
				
			$data = array("verified"=>'1',"modifiedDate"=>$datetime);	
            $result = $this->Mdl_badge->update("visitor_mobileotpverification",array("otp"=>$otp,"mobile_no"=>$mobile_no,"visitor_id"=>$visitor_id,"registration_id"=>$registration_id),$data);
			//echo $this->db->last_query();  exit;
                $response=array(               
                "Result"=>array("participant_Type"=>"VIS","mobile_no"=>$mobile_no,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id),
                "Message"=>"OTP has been verified successfully ",
                "status"=>"true"
                );				  
            }else{
          ...
          $response=array(
            ...
            "Result"=>array(),
            "Message"=>"Something wrong in Parameter",
            "status"=>"false"
          );  
        }
	} else {
	...
	/* Visitor PAN STATUS API */ 
	function checkVisitorPanStatus()
	{
	...
	if($this->checkAuth($obj))
	{
        ...
        */
		...
		//print_r($row); exit;
		$visitor_approval = $row[0]->visitor_approval;
		$visitor_name = $row[0]->name." ".$row[0]->lname;
		...
		if($visitor_approval !==""){
    		if($visitor_approval =="U"){ 
    					$vstatus = "Updated";
    		}else if($visitor_approval =="P"){
    					$vstatus = "Pending";
    		}else if($visitor_approval =="Y"){
    					$vstatus = "Approved";
    		} else if($visitor_approval =="D"){
    					$vstatus = "Disapproved";
    		}
 		}
			if($visitor_approval =="Y")
			{				
				$response=array(            
                "Result"=>array("participant_Type"=>"VIS","company_name"=>$company_name,"visitor_name"=>$visitor_name,"designation"=>$designation,"mobile_no"=>$mobile_no,"pan_no"=>$pan_no,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id,"status"=>$vstatus),
                "Message"=>"",
                "status"=>"true"
                );
				...
				$response=array(              
                "Result"=>array("participant_Type"=>"VIS","visitor_name"=>$visitor_name,"pan_no"=>$pan_no,"status"=>$vstatus),
                "Message"=>"",
                "status"=>"true"
                );  
			}
	...
	/* Visitor Direct Badge Download */
	function directVisitorBadge(){
	  $json = file_get_contents('php://input');
	  $obj = json_decode($json,True);
	  $pan_no = trim($obj['pan_no']);
	  $datetime = date("Y-m-d H:i:s");
	  ...
	  if($pan_no !=""){
        ...
        */
        ...
        if($panExist ===TRUE){
            ...
            $getOtpDetails =  $this->Mdl_badge->retrieve("globalExhibition",array("pan_no"=>$pan_no));           
            if($getOtpDetails !="NA"){
				$id = trim($getOtpDetails[0]->id);
            $data = array("isVerified"=>'1',"modified_date"=>$datetime,"otp_source"=>'app');
            $result=$this->Mdl_badge->update("globalExhibition",array("pan_no"=>$pan_no,"id"=>$id),$data);
                $uniqueId = trim($getOtpDetails[0]->uniqueIdentifier);
          ...
          $response=array(
            ...
            "status"=>"false"
			  ...
			  $response=array(
				"Result"=>array(),
				"Message"=>"Enter PAN Number properly",
				"status"=>"false"
			);  
		}
		}else{
	...
	function intlVisitorLogin()
	{
	  $json = file_get_contents('php://input');
	  $obj = json_decode($json,True);
	  $email_id = trim($obj['email']);
	  $datetime = date("Y-m-d H:i:s");

    if($this->checkAuth($obj)){
    if($email_id !=""){    
        /*
        ** CHECK EMAIL-ID NUMBER IS ALREADY EXIST IN TABLE OR NOT
        */
		...
		$row = $this->Mdl_badge->customQuery($sql);
		$company_name = strtoupper(str_replace('&amp;', '&', $row[0]->company_name));
		$first_name = $row[0]->first_name;
		$last_name = $row[0]->last_name;
		$visitor_name = $first_name.' '.$last_name;
		$designation = $row[0]->designation;
		$photo = $row[0]->photograph_fid;
        $registration_id = $row[0]->uid;
        $visitor_id = $row[0]->eid;
		$visitor_approval = $row[0]->application_approved;
		
		if($visitor_approval =="Y")
		{
         
			$data = array("otp"=>$otp,"verified"=>'0',"modified_date"=>$datetime);
            $result = $this->Mdl_badge->update("ivr_registration_details",array("email"=>$email_id,"eid"=>$visitor_id,"uid"=>$registration_id),$data);
			if($result){  
                ...
                $subject = "IIJS TRITIYA 2023 - Email ID Verification APP"; 
			    $cc = "";
			    $emailSend = array($email_id,"naheed@gjepcindia.com");
			...
			//	$photo="https://registration.gjepc.org/images/employee_directory/".$registration_id."/photo/".$photo_path;
                $response=array(               
                "Result"=>array("participant_Type"=>"INTL","company_name"=>$company_name,"visitor_name"=>$visitor_name,"designation"=>$designation,"email"=>$email_id,"otp"=>$otp,"registration_id"=>$registration_id,"visitor_id"=>$visitor_id),
                "Message"=>"OTP has been Sent to Registered Email id",
                "status"=>"true"
                );				
               } else {
               $response=array(               
              ...
              $response=array(               
                "Result"=>array(),
                "Message"=>"Error : Email id may be Disapproved,Pending",
                "status"=>"false"
              );  
        } 
        } else {
		...
		/******************************** Verify INTL Login OTP *****************************************************/
		
	function intlVisitorverifyOtp(){
	  $json = file_get_contents('php://input');
	  $obj = json_decode($json,True);
    ...
    {
        /*
        ** CHECK EMAIL ID IS ALREADY EXIST IN TABLE OR NOT
        */
		$sql = "SELECT * FROM ivr_registration_details WHERE otp='$otp' AND email='$email' AND eid='$visitor_id' AND uid = '$registration_id'";
        $panExist = $this->Mdl_badge->isExistCustom($sql);
        if($panExist === TRUE){
		$data = array("uid"=>$registration_id,"eid"=>$visitor_id);
           $getOtpDetails = $this->Mdl_badge->retrieve("ivr_registration_details",array("email"=>$email,"otp"=>$otp,"uid"=>$registration_id,"eid"=>$visitor_id));    
		//	echo $this->db->last_query();  exit;		   
           if($getOtpDetails !="NA"){            
            $db_otp = trim($getOtpDetails[0]->otp);
            if($db_otp == $otp){
            ...
            $result = $this->Mdl_badge->update("ivr_registration_details",array("otp"=>$otp,"email"=>$email,"eid"=>$visitor_id,"uid"=>$registration_id),$data);
			//echo $this->db->last_query();  exit;
                $response=array(               
                "Result"=>array("participant_Type"=>"INTL","email"=>$email,"uid"=>$registration_id,"eid"=>$visitor_id),
                "Message"=>"OTP has been verified successfully ",
                "status"=>"true"
                );			  
            } else{
		  ...
		  $response=array(
			"Result"=>array(),
			"Message"=>"Enter Email AND OTP properly",
			"status"=>"false"
		);  
	}
	}else{
	  ...
	  echo json_encode(array("Response"=>$response));
		...
		/* INTL Visitor Direct Badge Download */
	function directINTLVisitorBadges(){
	  $json = file_get_contents('php://input');
	  $obj = json_decode($json,True);
	  $email = trim($obj['email']);
	  $datetime = date("Y-m-d H:i:s");
	  ...
	  if($email !=""){
        ...
        ** CHECK MOBILE NUMBER IS ALREADY EXIST IN TABLE OR NOT
        */
		$sql = "SELECT * FROM gjepclivedatabase.globalExhibition where 1 AND email='$email' AND participant_Type='INTL'";
        $emailExist = $this->Mdl_badge->isExistCustom($sql);
        if($emailExist ===TRUE)
		{				
            $getOtpDetails =  $this->Mdl_badge->retrieve("globalExhibition",array("email"=>$email));           
            if($getOtpDetails !="NA"){
				$id = trim($getOtpDetails[0]->id);
            $data = array("isVerified"=>'1',"modified_date"=>$datetime,"otp_source"=>'app');
            $result=$this->Mdl_badge->update("globalExhibition",array("email"=>$email,"id"=>$id),$data);
                $uniqueId = trim($getOtpDetails[0]->uniqueIdentifier);
