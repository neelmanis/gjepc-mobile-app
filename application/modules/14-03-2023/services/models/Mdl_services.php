      ...
      $delete = $this->_deleteCat('feed_Category',$catId);
	   ...
	   function getEventBannerAndLinks(){
		...
		function saveContactDetailsLog($userUniqueId,$clientUniqueId)
		{
			...
			$clientQuery = "select gEx.id,gEx.uniqueIdentifier,gEx.fname,gEx.email,gEx.pan_no,gEx.mobile,gEx.participant_Type from globalExhibition as gEx where uniqueIdentifier = '$clientUniqueId'"; 
			$userQuery = "select gEx.id,gEx.uniqueIdentifier,gEx.fname,gEx.email,gEx.pan_no,gEx.mobile,gEx.participant_Type from globalExhibition as gEx where uniqueIdentifier = '$userUniqueId'"; //who is scannig doing
			$clientResult=$this->customQuery($clientQuery);
			$userResult=$this->customQuery($userQuery);
			$strResult = '';
			if(empty($userResult) || $userResult == "No Data"){
				return $strResult=array("Result"=>"fail",
								"Message"=>"User Data Not Found.",
								"status"=>"false");	
			}
			if(empty($clientResult) || $clientResult == "No Data"){
				return $strResult=array("Result"=>"fail",
								"Message"=>"Client Data Not Found.",
								"status"=>"false");	
			}

			if(!empty($clientResult) && !empty($userResult) && $userResult != "No Data" && $clientResult != "No Data"){
				$insertQuery=array(
					"userUniqueId"=>$userUniqueId,
					"clientUniqueId"=>$clientUniqueId,
					"client_name"=>$clientResult[0]->fname,
					"client_email"=>$clientResult[0]->email,
					"client_mobile"=>$clientResult[0]->mobile,
					"client_panno"=>$clientResult[0]->pan_no,
					"client_participant_Type"=>$clientResult[0]->participant_Type,
					"user_name"=>$userResult[0]->fname,
					"user_email"=>$userResult[0]->email,
					"user_mobile"=>$userResult[0]->mobile,
					"user_panno"=>$userResult[0]->pan_no,
					"user_participant_Type"=>$userResult[0]->participant_Type,
					"created_at" => date('Y-m-d H:i:s')
				);
				$strResult=$this->_insert('contact_details_scan_log',$insertQuery);
				if($strResult)
				{
					$strResult=array("Result"=>"success",
									"Message"=>"Data Inserted Successfully.",
									"status"=>"true");	
				}else
				{
					$strResult=array("Result"=>"fail",
									"Message"=>"some error occure.",
									"status"=>"false");	
				}
