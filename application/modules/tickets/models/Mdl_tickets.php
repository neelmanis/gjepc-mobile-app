...
class Mdl_tickets extends Mdl_generic {
  ...
  public function _get_datatables_query_tickets(){
    ...
    if($this->input->post('vendor_statuses')){
      $this->db->where('vendor_status_id', $this->input->post('vendor_statuses'));
    }
	...
	// $this->db->where('division_no',$division_no);
	 $this->db->where("FIND_IN_SET(division_no,'$division_no')>",'0');
	}  else {
    if(isset($admin_id)){
	...
	/* Overdue Tickets */
	var $table_overduetickets = "tickets";
	var $column_order_overduetickets = array("unique_code","exhibitor_code","exhibitor_name","hall_no","status_id"); 
	var $column_search_overduetickets = array("exhibitor_code","exhibitor_name"); 
	var $order_overduetickets = array("created_at"=>"desc"); 

  public function _get_datatables_query_overduetickets(){
    $admin = $this->session->userdata('admin');
	$role = $admin['role'];
    $admin_id = $admin['admin_id'];
    $division_no = $admin['division_name'];
	...
	$this->db->where("created_at <='$date' - INTERVAL 15 MINUTE");
